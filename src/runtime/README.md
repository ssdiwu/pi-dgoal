...
内部模块按职责拆分：`proposal.ts` 负责 proposal 输入规范化与确定性校验，`plan-view.ts` 负责工具、持续浮层和 Modal 共用的 Plan 进度只读投影，`liveness.ts` 负责双层活性的阈值、结构化指纹、纯判定与 begin/activity/durable/complete/reset 状态迁移语义；`index.ts` 保留公共工具注册、命令与跨边界编排，并按原名称转导既有活性公共 helper。
...
`phase_check` / `goal_check` 只写带 revision 的 `CheckRecord`；`plan_update` 是 agent 可调用的 task / phase / goal 执行状态、完成和主动暂停写入口。全局 Plan revision 保护 goal check；task/phase 的受审事实变化只递增所属 phase revision 并使该 phase check 失效。用户命令与技术熔断仍可暂停/恢复。Task Plan 的 `no_progress` 可由新的 `task_plan` 原子替换或完成守卫通过的显式 goal 收口解除；Task Plan 的 `model_error` 可由下一条 goal ID + 原始 prompt 精确绑定的真实 interactive/RPC 输入恢复同一 Plan，不额外投递 synthetic prompt。其他暂停原因及 Phase/Goal Plan 仍保持锁定。
