...
冷会话中，若 Goal Runtime 已记录当前真实用户明确要求使用/启动 dgoal 的一次性授权，`dgoal_propose` 不设置 `implicit` 即可消费授权并创建普通 pending goal；之后仍走相同结构校验、语义预审与用户确认 UI。该路径支持完整策略与计划内外部动作，但不扩张 `implicit=true` 的权限（ADR 0036）。
