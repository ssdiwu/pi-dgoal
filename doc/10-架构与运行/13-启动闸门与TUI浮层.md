...
- **每行**：`├─ [符] phase subject`，符 ○ pending / ◐ in_progress / ✓ done / ⚠ blocked。
...
决策依据：`doc/adr/0008-dgoal-s-modal-形态选型.md`（选定 Variant A=top-center overlay + scroll + 颜色 + emoji）；探索过程：`doc/20-能力参考/25-dgoal-s-modal变体探索参考.md`。

形态：
- **heading 钉顶**：`🎯 <objective 首行> (X/Y) ⏱️ <elapsed>`，accent 色 + bold
- **body 可滚动**：每 phase 一行（前缀 emoji `✅/🔄/⬜/🚧` + status 色），phase 下 task 缩进（`│    ✓/◐/○/⚠` + dim/accent/warning/muted 色）
- **底部 hint**：offset 指示 + 键位
...
- **空状态**：没有 active goal 时也弹同一个 top-center modal，显示“当前没有进行中的 dgoal”和 `/dgoal <goal>` 引导；非 TUI / custom 不可用时降级为 notify。
