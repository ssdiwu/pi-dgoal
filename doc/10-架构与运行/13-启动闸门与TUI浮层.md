...
dgoal_propose execute：存参数 + 触发确认 UI
  ↓
弹 ctx.ui.select 确认 UI（列出 goal + phases）：
...
- **task 默认隐藏**：双可见性轴。用户 `Ctrl+O` 展开看 phase 下 task 细节（含 blockedReason、evidence）。
- **completed 闪现**：两个 Set（pendingHide / hidden），render 记新完成，`agent_start` 搬进 hidden——dgoal 一轮轮跑，用户最想确认"上阶段成了"，这机制价值最高。
