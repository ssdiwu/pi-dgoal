...
决策依据：形态选型 `doc/adr/0008-dgoal-s-modal-形态选型.md`（Variant A=top-center overlay + scroll）；视觉编码 `doc/adr/0009-tui-visual-encoding-layer-over-status.md`（**层级靠颜色，状态靠字符**，覆盖 ADR 0008 的 emoji+status 色方案）；探索过程：`doc/20-能力参考/25-dgoal-s-modal变体探索参考.md`。
