...
**maker/checker split applied to the stop condition。** Addy 一句话点透 dgoal 建检循环核心——"停止条件本身套用制作器/检验器分离，判定完成的全新模型，不是写代码的那个"。dgoal 已落成 `dgoal_check` 独立子进程，且比内建 `/goal` 更深：全新上下文 + 受限核验工具 + phase 完成唯一入口 + 建检闸门锁定。关键辨析："fresh" 指上下文不是模型——dgoal 模型可复用（继承会话模型或 `auditorModel` 单独选模），关键是上下文隔离。印证 ADR 0006 + 22-ADaPT 参考。
