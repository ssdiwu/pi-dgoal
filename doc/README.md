...
8. `40-版本实施方案/41-v0.2.0-TaskPlan与建检循环实施方案.md` — 0.2.0 准备实施方案（8 个垂直切片）
9. `../index.ts` — 扩展入口，单文件实现

需要了解设计依据和外部参考时：

10. `adr/` — 架构决策记录（0001-0010），0006 是基本盘，0007 定 verification 必填，0008 是 `/dgoal s` modal 形态选型，0009 是 TUI 视觉编码改为“层级靠颜色，状态靠字符”，0010 是 done phase 软遗忘（注入只留标题行）
