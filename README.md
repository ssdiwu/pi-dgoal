...
| Tool | Purpose |
|---|---|
| `dgoal_propose` | Startup gate: submit goal + phases + initial tasks + frozen goal `acceptanceCriteria` (phase criteria required only for `phased`) + policy/budget recommendations + optional context/user review items. Explicit starts are user-confirmed; globally authorized lightweight starts may auto-confirm. |
